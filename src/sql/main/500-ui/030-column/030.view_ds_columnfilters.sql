...
select 
    concat(  'Tualo/DataSets/grid/filters/filter/TagField_',lower(ds_dropdownfields.table_name),'/',UCASE(LEFT(ds_dropdownfields.name, 1)), lower(SUBSTRING(ds_dropdownfields.name, 2))  ,'.js') filename,
    concat(
        'Ext.define(',doublequote(concat('Tualo.DataSets.grid.filters.filter.TagField_',lower(ds_dropdownfields.table_name),'.',UCASE(LEFT(ds_dropdownfields.name, 1)), lower(SUBSTRING(ds_dropdownfields.name, 2))  )),',',
            JSON_OBJECT(
                 "extend",  "Tualo.js.grid.filters.filter.TagFieldFilter",
                -- "extend", "Ext.grid.filters.filter.List",
                "alias", concat('grid.filter.',lower(concat(ds_dropdownfields.table_name,'_',ds_dropdownfields.name,'_tagfield_filter'))),
                "tablename", `ds_dropdownfields`.`table_name`,
                "idField", lower( concat( /*`ds_dropdownfields`.`table_name`,'__',*/ `ds_dropdownfields`.`idfield` )),
                "labelField", lower( concat( /*`ds_dropdownfields`.`table_name`,'__',*/ `ds_dropdownfields`.`displayfield` )),
                "store", JSON_OBJECT(
