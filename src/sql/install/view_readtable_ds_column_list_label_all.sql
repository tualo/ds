...
CREATE
OR REPLACE VIEW `view_readtable_ds_column_list_label_all` AS
select
  ifnull(
  ...
  ifnull(`ds_column_list_label`.`formatter`, '') AS `formatter`,
  ...
  if(
    `ds_column_list_label`.`column_name` is null,
    1,
    0
  ) AS `__virtual`
from
  (
    ...
    left join `ds_column_list_label` on(
      (
        (
          `ds_column_list_label`.`table_name` = `ic`.`table_name`
        )
        and (
          `ds_column_list_label`.`column_name` = `ic`.`column_name`
        )
      )
    )
  );

call fill_ds('ds_column_list_label');

call fill_ds_column('view_readtable_ds_column_list_label_all');

call fill_ds_column('ds_column_list_label');