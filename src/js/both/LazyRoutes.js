Ext.define('Tualo.routes.LazyDS', {
    statics: {
            ...
            if (response.success == true) {
                for (let i = 0; i < response.data.length; i++) {
                    if (!Ext.isEmpty(response.data[i].table_name))
                        list.push({
                            name: response.data[i].title + ' (' + '#lazyds/' + response.data[i].table_name + ')',
                            path: '#lazyds/' + response.data[i].table_name
                        });
                }
            }
            return list;
    ...
    url: 'lazyds/:{type}/:{table}(\/:{id})',
    handler: {
        action: function (values) {
            console.log('lazyds route', 'action', values);
            ...
            const fnx = async () => {
                let type = values.type,
                    ...
                    component = null,
                    cmp_id = 'lazy_' + type + '_' + tablename.toLowerCase() + '_' + (await Tualo.routes.LazyDS.sha1(JSON.stringify(values)));

                console.log('lazyds route', 'action', 'cmp_id', cmp_id);
                ...
                if (!Ext.isEmpty(component)) {
                    ...
                    component = Ext.getApplication().addView('Tualo.ds.lazy.Panel', {
                        tablename: tablename,
                        displaytype: type,
                        dsID: values.id,
                        itemId: cmp_id
        ...
        before: function (values, action) {
            ...
            const fnx = async () => {
                let type = values.type,
                    tablename = values.table,
                    tablenamecase = tablename.toLocaleUpperCase().substring(0, 1) + tablename.toLowerCase().slice(1),
                    cmp_id = 'lazy_' + type + '_' + tablename.toLowerCase() + '_' + (await Tualo.routes.DS.sha1(JSON.stringify(values)));
