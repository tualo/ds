...
select `ds_column`.`table_name`,
    ...
    --    ) obj
from ds_column
    join ds_column_form_label on (ds_column.table_name, ds_column.column_name) = (
...
select `ds_column_form_label`.`table_name`,
    ...
    if(
        ...
        JSON_OBJECT(
            ...
            'xtype',
            if(
                view_readtable_all_types_modern.type is null,
                'missedxtypefield',
        ...
        JSON_OBJECT(
            ...
            group_concat(
                `ds_column_form_label`.`label`
                ORDER BY `ds_column_form_label`.`position` separator ' | '
            ),
            'xtype',
            'fieldcontainer',
            'layout',
...
where `ds_column_form_label`.`active` = 1
    and `ds_column_form_label`.`hidden` = 0 --    and `ds_column_form_label`.`table_name` = 'adressen'
