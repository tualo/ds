    ...
    IF JSON_TYPE(JSON_EXTRACT(request,'$.data'))='ARRAY' THEN 
        ...
        IF use_table_name is not null THEN 
            ...
            select 
                ...
                group_concat(
                    concat('`',column_name,'` ',if(INSTR(column_type,'enum')=1,'varchar(255)', column_type),' path "$.', column_name,'" ','NULL' ,' ON EMPTY ')
                    order by column_name
                    separator ','
                ) x,
                group_concat(
            ...
            drop TEMPORARY table if exists temp_dsx_rest_data;
