...
CREATE OR REPLACE FUNCTION `dsx_rest_api_set_get_update_statement`(IN  request JSON, IN  temp_table_name varchar(128), IN  use_table_name varchar(128)) returns LONGTEXT
BEGIN

    DECLARE use_template_update_fields LONGTEXT;
    DECLARE sql_command LONGTEXT;
    select
        group_concat( concat('`',use_table_name,'`.`',column_name,'` = temp_alias.`',column_name,'`') separator ',') s
    into 
    ...
    -- dsx_rest_api_set_get_update_statement
    RETURN sql_command;

END //

CREATE OR REPLACE FUNCTION `dsx_rest_api_set_get_delete_statement`( IN  request JSON, IN  temp_table_name varchar(128), IN  use_table_name varchar(128)) returns LONGTEXT
BEGIN
    DECLARE sql_command LONGTEXT;
    select 
        concat('
...
BEGIN
    ...
    IF JSON_VALUE(request,'$.type')='update' THEN
        SET sql_command = dsx_rest_api_set_get_update_statement(request,temp_table_name,use_table_name);
    END IF;
